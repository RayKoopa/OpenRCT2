                            ...
                            && Config::Get().general.autoOpenShops)
                        {
                            ...
                            gameAction.SetCallback(
                                [ride](const GameActions::GameAction* ga, const GameActions::Result* result) {
                                    ...
                                    auto* windowMgr = GetWindowManager();
                                    const auto rideId = ride->id.ToUnderlying();
                                    if (windowMgr->FindByNumber(WindowClass::rideConstruction, rideId) != nullptr)
                                        windowMgr->CloseByNumber(WindowClass::ride, rideId);
                                });
